...
void filesystem_info_t_construct(filesystem_info_t* filesystem, OSPiHandle* handler, char* segment) {
    ...
    dma_object_t_construct(&filesystem->dma_obj, handler, segment);
    ...
    for (i = 0x1500; filesystem->fs_addr == 0; i += 8) {
        ...
        dma_object_t_read_rom(&filesystem->dma_obj, i, read_in_data, 8);
    ...
    dma_object_t_read_rom(&filesystem->dma_obj, i, &filesystem->num_files, sizeof(uint64_t));
    ...
    dma_object_t_read_rom(&filesystem->dma_obj, i, &filesystem->archive_size, 8);
...
uint32_t filesystem_info_t_get_index_from_hash(uint64_t hash, filesystem_info_t* filesystem) {
    uint32_t index = 0;
    ...
    for (index = 0; index < filesystem->num_files; index++) {
        dma_object_t_read_rom(&filesystem->dma_obj, filesystem->file_hashes + (index * sizeof(uint64_t)), &rhash, sizeof(uint64_t));
...
uint32_t filesystem_info_t_get_offset_from_index(uint32_t index, filesystem_info_t* filesystem) {
    uint32_t offset __attribute__((aligned (8))) = 0;

    dma_object_t_read_rom(&filesystem->dma_obj, filesystem->file_offsets + (index * sizeof(uint32_t)), &offset, sizeof(uint32_t));

    return offset;
}

void* filesystem_info_t_read_file(const char* file, void* dest, filesystem_info_t* filesystem) {
    uint64_t file_size = 0;
    uint64_t hash = djb2_hash(file);
    uint32_t index = filesystem_info_t_get_index_from_hash(hash, filesystem);
    uint32_t offset = 0;

    if (index < filesystem->num_files) {
        if (index == (filesystem->num_files - 1)) {
            offset = filesystem_info_t_get_offset_from_index(index, filesystem);
            file_size = filesystem->archive_size - offset;
        }
        else {
            offset = filesystem_info_t_get_offset_from_index(index, filesystem);
            file_size = filesystem_info_t_get_offset_from_index(index + 1, filesystem) - offset;
        }

        osInvalDCache(dest, file_size);
        dma_object_t_read_rom(&filesystem->dma_obj, filesystem->archive + offset, dest, file_size);
    }
    else {
